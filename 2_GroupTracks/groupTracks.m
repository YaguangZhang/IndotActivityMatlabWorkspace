...
% Synthesized GPS sampling rate in Hz after location record interpolation.
TARGET_GPS_SAMP_RATE_IN_HZ = 1;
targetGpsSampTimeInS = 1/TARGET_GPS_SAMP_RATE_IN_HZ;
...
for idxDay = 1:numOfDays
    disp(['    [', datestr(now, datetimeFormat), ...
        '] Day #', num2str(idxDay), '/', num2str(numOfDays), '...'])
    ...
    for idxVeh = 1:curNumOfVehs
        ...
        for idxCleanTrack = 1:curNumOfCleanTs
            ...
            localTimestampRanges{idxVeh}{idxCleanTrack} ...
                = curGpsLocTrackTable.timestamp_local([1,end])';
        end
    end
    ...
    for idxVeh = 1:curNumOfVehs
        ...
        for idxCleanTrack = 1:curNumOfCleanTs
            curLocalTimestampRange ...
                = localTimestampRanges{idxVeh}{idxCleanTrack};
            curVehUtmXYs = horzcat( ...
                closeByVehsEachDay{idxDay}{idxVeh} ...
                {idxCleanTrack}.utm_x, ...
                closeByVehsEachDay{idxDay}{idxVeh} ...
                {idxCleanTrack}.utm_y);

            for idxOtherVeh = (idxVeh+1):curNumOfVehs
                curOtherVehId = gpsLocTableCleanTracksEachDay ...
                    {idxDay}{idxOtherVeh}{1}.vehicleId(1);
                for idxOtherCleanTrack = 1:length( ...
                    ...
                    if minutes(curOverlapTimestampRange(2) ...
                        ...
                        curOtherVehUtmXYs = horzcat( ...
                            closeByVehsEachDay{idxDay}{idxOtherVeh} ...
                            {idxOtherCleanTrack}.utm_x,  ...
                            closeByVehsEachDay{idxDay}{idxOtherVeh} ...
                            {idxOtherCleanTrack}.utm_y);

                        % Interpolate the tracks for finer control over
                        % time. The time range should already start and end
                        % at integer second values, but just in case, we
                        % will clip the range if necessary to make sure.
                        interpedLocTimeSegStart ...
                            = dateshift(curOverlapTimestampRange(1), ...
                            'start', 'second');
                        if curOverlapTimestampRange(1) ...
                                >interpedLocTimeSegStart
                            interpedLocTimeSegStart ...
                                = interpedLocTimeSegStart + seconds(1);
                        end
                        interpedLocTimeSegEnd ...
                            = dateshift(curOverlapTimestampRange(2), ...
                            'start', 'second');

                        % Convert the timestamps to seconds past midnight
                        % for easier interpolation.
                        interpedTimeRangeInS ...
                            = seconds([interpedLocTimeSegStart ...
                            - localDateTimeThisDay, ...
                            interpedLocTimeSegEnd ...
                            - localDateTimeThisDay]);

                        curVehTimesInS = seconds( ...
                            gpsLocTableCleanTracksEachDay{idxDay} ...
                            {idxVeh}{idxCleanTrack}.timestamp_local ...
                            - localDateTimeThisDay);
                        curOtherVehTimesInS = seconds( ...
                            gpsLocTableCleanTracksEachDay ...
                            {idxDay}{idxOtherVeh} ...
                            {idxOtherCleanTrack}.timestamp_local ...
                            - localDateTimeThisDay);

                        % Linearly interpolate both tracks in UTM by time
                        % and compute the vehicle to other vehicle distance
                        % accordingly.
                        curVehSampIndices = find( ...
                            curVehTimesInS>=interpedTimeRangeInS(1) ...
                            & curVehTimesInS<=interpedTimeRangeInS(2));
                        curOtherVehInterpedUtmXYs = interp1( ...
                            curOtherVehTimesInS, curOtherVehUtmXYs, ...
                            curVehTimesInS(curVehSampIndices));
                        curDistVehToOtherVehInM = sqrt(sum(( ...
                            curVehUtmXYs(curVehSampIndices, :) ...
                            - curOtherVehInterpedUtmXYs ...
                            ).^2,2));

                        curOtherVehSampIndices = find( ...
                            curOtherVehTimesInS ...
                            >=interpedTimeRangeInS(1) ...
                            & curOtherVehTimesInS ...
                            <=interpedTimeRangeInS(2));
                        curVehInterpedUtmXYs = interp1( ...
                            curVehTimesInS, curVehUtmXYs, ...
                            curOtherVehTimesInS(curOtherVehSampIndices));
                        curDistOtherVehToVehInM = sqrt(sum(( ...
                            curOtherVehUtmXYs( ...
                            curOtherVehSampIndices, :) ...
                            - curVehInterpedUtmXYs ...
                            ).^2,2));

                        % Record the distance only if it is small enough.
                        for curIdxVehSampToUpdate = find( ...
                                curDistVehToOtherVehInM ...
                                <=MAX_CLOSE_BY_DIST_IN_M)'
                            closeByVehsEachDay{idxDay}{idxVeh} ...
                                {idxCleanTrack}.closeByVehsId{ ...
                                curVehSampIndices(curIdxVehSampToUpdate)};
                            closeByVehsEachDay{idxDay}{idxVeh} ...
                                {idxCleanTrack}.closeByVehsId{ ...
                                curVehSampIndices ...
                                (curIdxVehSampToUpdate)} = horzcat( ...
                                closeByVehsEachDay{idxDay}{idxVeh} ...
                                {idxCleanTrack}.closeByVehsId{ ...
                                curVehSampIndices...
                                (curIdxVehSampToUpdate)}, curOtherVehId);
                        end

                        for curIdxOtherVehSampToUpdate = find( ...
                                curDistOtherVehToVehInM ...
                                <=MAX_CLOSE_BY_DIST_IN_M)'
                            closeByVehsEachDay{idxDay}{idxOtherVeh} ...
                                {idxOtherCleanTrack}.closeByVehsId{ ...
                                curOtherVehSampIndices( ...
                                curIdxOtherVehSampToUpdate)} = horzcat( ...
                                closeByVehsEachDay{idxDay}{idxOtherVeh} ...
                                {idxOtherCleanTrack}.closeByVehsId{ ...
                                curOtherVehSampIndices( ...
                                curIdxOtherVehSampToUpdate)}, ...
                                curOtherVehId);
                        end
                    end
                end
